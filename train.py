...
# os.environ['GLOG_v'] = '3'  # 0-DEBUG、1-INFO、2-WARNING、3-ERROR
...
def DownloadFromQizhi(obs_data_url, data_dir):
    ...
    if device_num > 1:
        ...
        context.set_context(mode=context.GRAPH_MODE, device_target=opt.device_target, device_id=int(os.getenv('ASCEND_DEVICE_ID')))
...
class UploadOutput(Callback):
    def __init__(self, train_dir, obs_train_url, execute_every_n_epochs=1):
        self.train_dir = train_dir
        self.obs_train_url = obs_train_url
        self.execute_every_n_epochs = execute_every_n_epochs
        self.epoch_counter = 0
    def epoch_end(self, run_context):
        self.epoch_counter += 1
        if self.epoch_counter % self.execute_every_n_epochs == 0:
            try:
...
def set_parallel_context(args_opt):
    ...
    context.set_auto_parallel_context(
        ...
        full_batch=bool(args_opt.full_batch), strategy_ckpt_load_file=os.path.join(args_opt.strategy_load_ckpt_path, "strategy.ckpt"),
        enable_parallel_optimizer=bool(args_opt.optimizer_shard), strategy_ckpt_save_file=os.path.join(args_opt.save_checkpoint_path, "strategy.ckpt"),
...
def run_train(args_opt):
    ...
    # Set model property
    model_parallel_num = args_opt.op_level_model_parallel_num          # 2
    data_parallel_num = int(device_num / model_parallel_num)           # 8
    print('device_num', device_num)
    print('model_parallel_num', model_parallel_num)
    print('data_parallel_num', data_parallel_num)
    ...
    print('get_dataset_size', step_per_epoch)
    ...
    if args_opt.pre_trained:
        ...
        print("[has_trained_epoch] is: ", loss_callback.has_trained_epoch, flush=True)
        print("[has_trained_step] is: ", loss_callback.has_trained_step, flush=True)
...
def restore_checkpoint(args_param, sink_size, dataset, model, network, epoch):
    ...
    ckpt_pattern = os.path.join(args_param.load_ckpt_path, "rank_{}".format(D.get_rank()),
                                f"{ckpt_name}*.ckpt")
    ...
    ckpt_exp_pattern = os.path.join(args_param.load_ckpt_path, "rank_{}".format(D.get_rank()),
    ...
    if param_dict.get("epoch_num") and param_dict.get("step_num"):
        ...
        print('get_epoches:', int(param_dict["epoch_num"].data.asnumpy()))
        print('get_steps:', int(param_dict["step_num"].data.asnumpy()))
...
def get_exception_checkpoints(args_param):
    ...
    for ckpt_rank in restore_rank_list:
        ckpt_pattern = os.path.join(args_param.load_ckpt_path,
                                    f"rank_{ckpt_rank}",
                                    f"{ckpt_name}*_breakpoint.ckpt")
        ckpt_files = glob.glob(ckpt_pattern)
        if not ckpt_files:
            print(
                f"There is no ckpt file in {args_param.load_ckpt_path}, "
                f"current ckpt_files found is {ckpt_files} "
                f"with pattern {ckpt_pattern}, so skip the loading.")
            return None
        ckpt_files.sort(key=os.path.getmtime, reverse=True)
...
def run_train_pipeline(args_opt):
    ...
    model_parallel_num = args_opt.op_level_model_parallel_num    # 1
    stage_device_num = int(device_num / args_opt.stage_num)      # 8
    data_parallel_num = int(stage_device_num / model_parallel_num)  # 8
    is_last_stage = (rank_id // stage_device_num) == args_opt.stage_num -1
    per_batch_size = args_opt.per_batch_size   # 4
    batch_size = per_batch_size * data_parallel_num * args_opt.micro_size  # 64
    micro_batch_interleaved = args_opt.micro_batch_interleaved
    recompute_config = TransformerRecomputeConfig(recompute=True,
                                                  recompute_slice_activation=bool(args_opt.recompute_slice_activation))
    parallel_config = TransformerOpParallelConfig(data_parallel=data_parallel_num, model_parallel=model_parallel_num,
    ...
    # uploadOutput = UploadOutput(args_opt.save_checkpoint_path, args_opt.train_url, execute_every_n_epochs=1000)
    # print('save_checkpoint_path', args_opt.save_checkpoint_path)
    # print('train_url', args_opt.train_url)
    ...
    # UploadToQizhi(opt.save_checkpoint_path, opt.train_url)
...
if __name__ == "__main__":
    ...
    # DownloadFromQizhi(opt.data_url, data_dir)
    # opt.data_url = data_dir
    # opt.save_checkpoint_path = opt.train_url
    # add end
    ...
    print('opt.hccl_connect_time', opt.hccl_connect_time)
    if opt.stage_num > 1:
