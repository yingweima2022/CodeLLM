...
class PanGUAlphaWithLoss1(Cell):
    """
    PanguAlpha training loss for generation.
    Args:
        config(PanGUConfig)
    ...
    def __init__(self, config, network, loss):
        super(PanGUAlphaWithLoss1, self).__init__(auto_prefix=False)
        self.batch_size = config.batch_size
        self.seq_length = config.seq_length
        dp = config.parallel_config.data_parallel
        self.network = network
    ...
    def construct(self, input_ids, label_ids, input_position=None, attention_mask=None):
        r"""Forward process of the pangu alpha model"""
        tokens = self.slice(input_ids, (0, 0), (self.batch_size, -1), (1, 1))
        labels = self.slice(label_ids, (0, 1), (self.batch_size, self.len + 1), (1, 1))
        input_position = self.slice(input_position, (0, 0), (self.batch_size, self.len), (1, 1))
        decoder_attention_masks = self.slice2(attention_mask, (0, 0, 0), (self.batch_size, self.len, self.len), (1, 1, 1))
        
        # input_mask <=> labels左移一位
        input_mask = F.cast(self.not_equal(labels, self.pad_token), mstype.float32)
