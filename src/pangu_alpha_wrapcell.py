...
#instruction tuning
class PanguAlphaTrainOneStepWithLossScaleCell1(TrainOneStepWithLossScaleCell):
    """
    Encapsulation class of PanguAlpha network training.
    ...
    def __init__(self,
                 ...
                 enable_global_norm=False,
                 config=None):
        super(PanguAlphaTrainOneStepWithLossScaleCell1,
              self).__init__(network, optimizer, scale_update_cell)
        self.network = network
        self.config = config
        self.weights = optimizer.parameters
    ...
    def construct(self, input_ids, label_ids, input_position, attention_mask, layer_past=None, sens=None):
        """Defines the computation performed."""
        weights = self.weights
        # Forward process
        loss = self.network(input_ids, label_ids, input_position, attention_mask)
        scaling_sens = self.scale_sense
