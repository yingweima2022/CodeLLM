...
def compute_loss(logits, labels, mask):
    ...
    loss = -select * mask
    num_valid_loss = np.sum(mask)
    loss = np.sum(loss)
    # avg_loss = np.mean(loss)
    avg_loss = loss / num_valid_loss
    # ppl 添加
    ppl = np.exp(avg_loss)
    return ppl

def compute_ppl(logits, labels, mask):
    labels = labels.astype(np.int32)
    select = gather(logits, labels[0])
    loss = np.log(np.maximum(1e-10, -select)) * mask
    print('select', select)
    print('max', np.maximum(1e-10, -select))
    print('loss', loss)
    avg_loss = np.mean(loss)
    # ppl 添加
    ppl = np.exp(avg_loss)
...
def get_scores(model_predict, item, tokenizer, pad_length=1024):
    ...
    tokens = convert_text_to_ids(input_sentence, tokenizer, pad_length, pad=tokenizer.pad_token_id, plus=0)
    prompt_ids = convert_text_to_ids(prompt, tokenizer, pad_length, pad=tokenizer.pad_token_id, plus=0)
    labels = np.concatenate((tokens[:, 1:], np.ones((tokens.shape[0], 1)) * tokenizer.pad_token_id), axis=-1)
...
def get_answer(model_predict, item, tokenizer, args_opt, usepast=False, pad_length=1024):
    input_sentence = item['input_str']
    prompt = item['prompt']
    sample = input_sentence + prompt
    ...
    # Call inference
    # print('usepast', flush=True)
    generate_func = generate_increment if usepast else generate
...
def generate_increment(model, origin_inputs, config):
    ...
    frequency_list = np.array([[0 for _ in range(config.vocab_size)]])
    pad_length = seq_length - origin_inputs.shape[-1]
    if pad_length < 0:         # input超过输入限制
        pad_length = 0
        outputs.append(end_token)
