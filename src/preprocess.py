...
def task_unit(iterator, tokenizer, seq_length, eot, parallel_writer=True):
    ...
    while True:
        ...
        try:
            ...
            print("Process {} transformed {} records try.".format(
                index, count))
        except StopIteration:
            if data_batch:
                writer.write_raw_data(data_batch,
                                      parallel_writer=parallel_writer)
                print("Process {} transformed {} records. except".format(
                    index, count))
            break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_file', type=str,
                        default='./output/transfered_mindrecord_0.mindrecord')
...
#     # Start to load tokenizer
    if args.tokenizer == 'gpt':
        ...
        vocab_size = len(word_tokenizer.get_vocab())
        print("GPT-2 tokenizer vocab size:", vocab_size)
    else:
...
#             map_func = partial(task_unit, tokenizer=word_tokenizer, seq_length=args.seq_length, eot=args.eot, parallel_writer=False)
#             print('stage1')
...
#     print("Transform finished, output files refer: {}".format(out_file))
    ...
    out_file = args.output_file
    dataset = ds.MindDataset([out_file], columns_list=['input_ids'], shuffle=False, num_samples=None)
    column_names = dataset.get_col_names()
    ...
    for data in dataset.create_dict_iterator(output_numpy=True):
        ...
        for column_name in column_names:
            feature_or_label = data[column_name]
            # print(f"{column_name}: {feature_or_label}")
