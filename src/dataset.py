...
def create_dataset(batch_size, data_path, device_num=1, rank=0, drop=True, full_batch=False, data_start_index=0,
    ...
    data = []
    for dir in dirs:
        tdir = os.path.join(home_path, dir)
        files = os.listdir(tdir)
        data_temp = [
            os.path.join(home_path, dir, name) for name in files
    ...
    print('col_name', dataset.get_col_names())
...
# instruction_process  dataset
def get_input_data_batch_slice_map1(input_ids, label_ids, eod_id, rank, dis, eod_reset):
    """
    Generate position_id and attention_mask according to input_ids considering eod reset
    ...
    rank = int(rank)
    input_ids = input_ids[rank * dis: (rank + 1) * dis]
    label_ids = label_ids[rank * dis: (rank + 1) * dis]
    if not eod_reset:
    ...
    return batch_input_ids, batch_label_ids, batch_position_ids, batch_attention_mask


def create_dataset1(batch_size, data_path, device_num=1, rank=0, drop=True, full_batch=False, data_start_index=0,
                   eod_reset=False, eod_id=9, column_name='input_ids', column_name1='label_ids', epoch=1, num_samples=None):
    """
    Create dataset
    ...
    # Load data files and preprocess
    # print('data', data[data_start_index:])
    dataset = ds.MindDataset(data[data_start_index:], columns_list=[column_name, column_name1],
                             shuffle=False, num_samples=num_samples)
    ...
    map_func = (lambda input_ids, label_ids: get_input_data_batch_slice_map1(input_ids, label_ids, eod_id, rank, dis, eod_reset))
    # If eod_reset enabled, another two inputs will be generated through input_ids
    if eod_reset:
        dataset = dataset.batch(batch_size, drop_remainder=drop)
        dataset = dataset.map(operations=map_func, input_columns=[column_name, column_name1],
                              output_columns=[column_name, column_name1, "position_id", "attention_mask"],
                              column_order=[column_name, column_name1, "position_id", "attention_mask"])
        dataset = dataset.map(input_columns="position_id", operations=type_cast_op)
        dataset = dataset.map(input_columns="attention_mask", operations=type_cast_op_float)
    else:
        dataset = dataset.map(input_columns=[column_name, column_name1], operations=type_cast_op)
        dataset = dataset.batch(batch_size, drop_remainder=drop)
        dataset = dataset.map(operations=map_func, input_columns=[column_name, column_name1],
                              output_columns=[column_name, column_name1])
    dataset = dataset.map(input_columns=column_name, operations=type_cast_op)
    dataset = dataset.map(input_columns=column_name1, operations=type_cast_op)
    dataset = dataset.repeat(epoch)
    return dataset
