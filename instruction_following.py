...
from src.dataset import create_dataset, create_dataset1
from src.pangu_alpha import PanGUAlphaWithLoss, PanGUAlphaWithLoss1, PanguAlphaModel
from src.pangu_alpha_wrapcell import PanguAlphaTrainOneStepWithLossScaleCell, PanguAlphaTrainPipelineWithLossScaleCell, PanguAlphaTrainOneStepWithLossScaleCell1
...
def run_train(args_opt):
    ...
    pangu_alpha_with_loss_net = MicroBatchInterleaved(PanGUAlphaWithLoss1(config, pangu_alpha, loss),
    ...
    ds = create_dataset1(config.batch_size * micro_batch_interleaved, data_path=cache_url, data_start_index=0,
                        eod_reset=config.eod_reset, full_batch=bool(args_opt.full_batch), eod_id=args_opt.eod_id,
                        device_num=device_num, rank=rank, column_name=args_opt.data_column_name, column_name1='label_ids', epoch=epoch_num)
    step_per_epoch = ds.get_dataset_size()
    print('get_dataset_size', step_per_epoch)
    print('col_name', ds.get_col_names())
    print('batch size', ds.get_batch_size())
    ...
    pangu_alpha_with_grads = PanguAlphaTrainOneStepWithLossScaleCell1(
