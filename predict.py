...
from src.generate import get_scores, get_answer
...
def load_model(args_opt):
    ...
    if args_opt.eval_task!=None and args_opt.eval_task!='dureader' and args_opt.eval_task!='mbpp' : # and args_opt.eval_task!='all':
        print('set use past false', flush=True)
        use_past = False
    else:
        use_past = True if args_opt.export else (args_opt.use_past == "true")
        print('set use past select', use_past, flush=True)
    ...
    if args_opt.eval_task and args_opt.eval_task!='dureader' and args_opt.eval_task!='mbpp' : # and args_opt.eval_task!='all':
    ...
    if args_opt.distribute == "false":
        predict_layout = None
    elif args_opt.eval_task and args_opt.eval_task!='dureader' and args_opt.eval_task!='mbpp' : # and args_opt.eval_task!='all':
...
def run_eval(model_predict, config, args_opt):
    ...
    examples = load_dataset(args_opt.eval_task, split='validation', tokenizer=tokenizer, in_context=False,
                            data_url=args_opt.eval_data_url)
    # Tokenize input sentence to ids
    point = TimePoint()
    point.set_start()
    
    print(args_opt.eval_task, 'use_past is:', config.use_past, flush=True)
    if args_opt.eval_task == 'logic':
        f_math = open('logic_eval.txt', 'w')

    for item in tqdm(examples, total=len(examples)):
        if args_opt.eval_task == 'dureader' or args_opt.eval_task == 'mbpp':
            output_ids = get_answer(model_predict, item, tokenizer, args_opt, usepast=config.use_past, pad_length=config.seq_length)
        else:
...
def main():
    ...
    model_predict, config = load_model(opt)   # mbpp 等生成需要修改load_model条件
    ...
    if opt.eval_task:
        if opt.eval_task == 'all':
            # tasks = [ 'cosqa', 'ekar', 'jecqa', 'logic', 'scienceqa']  # 'c3', 'ocnli', 'cmnli', 'dureader'
            tasks = ['c3', 'ocnli', 'cmnli']  # , 'dureader'
            # tasks = ['mbpp', 'dureader']
            for task in tasks:
                opt.eval_task = task
                opt.eval_data_url = '/home/ma-user/work/notebook_code/data_downst/'+ task
                examples = run_eval(model_predict, config, opt)
                if opt.eval_task == 'dureader' or opt.eval_task == 'mbpp':
                    file_name = '/home/ma-user/work/notebook_code/openi/'+opt.eval_task+'_results_level.json'
                    with open(file_name, 'w') as f:
                        json.dump(examples, f)
                metric = load_metric(opt.eval_task)(examples)
                print(f"Metric for dataset {opt.eval_task} is {metric}", flush=True)
                with open('/home/ma-user/work/notebook_code/openi/scores/'+opt.eval_task+'.txt', 'w') as f:
                    f.write(f"Metric for dataset {opt.eval_task} is {metric}")
            return
        else:
            examples = run_eval(model_predict, config, opt)
            metric = load_metric(opt.eval_task)(examples)
            print(f"Metric for dataset {opt.eval_task} is {metric}")
            if opt.eval_task == 'dureader' or opt.eval_task == 'mbpp':
                file_name = '/home/ma-user/work/notebook_code/openi/'+opt.eval_task+'_results.json'
                with open(file_name, 'w') as f:
                    json.dump(examples, f)
            return 
